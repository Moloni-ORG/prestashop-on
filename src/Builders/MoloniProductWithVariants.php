...
use MoloniOn\Builders\MoloniProduct\Helpers\Variants\CreateMappingsAfterMoloniProductCreateOrUpdate;
use MoloniOn\Builders\MoloniProduct\Helpers\Variants\FindOrCreatePropertyGroup;
use MoloniOn\Builders\MoloniProduct\Helpers\Variants\GetOrUpdatePropertyGroup;
use MoloniOn\Builders\MoloniProduct\Helpers\Variants\UpdateMoloniVariantsProductImage;
use MoloniOn\Builders\MoloniProduct\ProductCategory;
use MoloniOn\Builders\MoloniProduct\ProductTax;
use MoloniOn\Builders\MoloniProduct\ProductVariant;
        ...
        try {
            ...
            if (!empty($moloniProduct)) {
                ...
                if ($this->shouldWriteLogs()) {
                    Logs::addInfoLog(
                        ['Product created in Moloni ON ({0})', ['{0}' => $this->reference]],
                        ['props' => $props]
                    );
                }
    ...
    public function update(): MoloniProductWithVariants
    {
        ...
        try {
            ...
            if ($productId > 0) {
                ...
                if ($this->shouldWriteLogs()) {
                    Logs::addInfoLog(
                        ['Product updated in Moloni ON ({0})', ['{0}' => $this->reference]],
                        ['props' => $props]
                    );
                }
